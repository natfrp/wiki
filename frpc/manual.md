...
#### auto_https_verify_client

即 `自动HTTPS访问域前置检查`，自 `0.51.0-sakura-10` 起，可通过该开关启用 HTTP 请求部分的 Host 头验证功能。

启用后，自动 HTTPS 功能在将请求转送给后端应用前，会检查请求的 Host 头是否与握手使用的参数匹配，对于不匹配的返回 HTTP 421。从而避免域前置攻击。

- 请将此功能与 [auto_https_policy](#auto_https_policy) 配合使用，以确保证书严格匹配
- 此功能的判断在 [plugin_host_header_rewrite](#plugin_host_header_rewrite) 之前进行，从而保护其仅在合法连接中生效

| 取值 | 说明 |
| :---: | --- |
| 留空<br>**[默认值]** | 不启用 Host 头验证功能 |
| true | 启用 Host 头验证功能 |
