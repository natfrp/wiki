...
首先您需要正确区分 **HTTP 隧道** 和 **TCP 隧道映射 HTTP 流量** 的区别。
...
如果您明白什么是 **HTTP 隧道** 和什么是 **TCP 隧道映射 HTTP 流量**，问题就很简单了。实名认证后，您可以解锁的权限是在国内节点 **使用 TCP 隧道映射 HTTP 流量** 以及在所有节点 **使用 HTTP(S) 隧道**

如果您想让其他人直接使用 `http://www.example.com` 访问您的网站，就必须进行实名认证，若您想使用 **国内节点** 建站还需要使用一个已经备案的域名。

如果您不想实名认证，可以选用海外节点 **使用 TCP 隧道映射 HTTP 流量**。
