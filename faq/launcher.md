...
### 重启后无法进入桌面 {#linux-stuck-login}

如果您配置了服务自动启动后，重启无法正常进入桌面，这可能是启动器的 Web UI 端口与 BRLTTY 的端口冲突导致的。

BRLTTY 是一个后台进程，它为使用盲文显示器的视障人士提供对 Linux 控制台的访问，某些发行版预装了该软件包。

您可以考虑下列方案之一：

- 如果您不是视障人士，请参考发行版的相关说明卸载 `brltty` 软件包。
- 更改启动器的 Web UI 监听端口。
