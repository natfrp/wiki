...
- 根据有关规定，使用 `TCP` 隧道时，如果您穿透的 **应用协议** 是 HTTP，**必须使用非内地节点**
- 根据有关规定，使用 `HTTP(S)` 隧道时，如果使用内地节点进行穿透，必须使用 **有备案** 的域名  
...
管理面板中能建立 `HTTP(S)` 隧道的节点会有绿色的 **建站** 标识，通常情况下所有节点都可以建立 TCP 隧道。
...
::: tip
协议为 HTTP 时，如果您需要 HTTPS 而本地服务不支持 HTTPS 协议，您也可以打开 [自动 HTTPS](/frpc/auto-https.md) 开关
:::
