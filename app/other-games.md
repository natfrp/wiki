...
夜族崛起使用 **UDP** 协议，所以我们需要配置一个 UDP 隧道：
 ...
 - 2. 在游戏目录的找到`VRising-Server`目录，进入后寻找启动文件，如果你是windows系统，寻找`.bat`结尾的文件，如果是linux，则寻找`.sh`结尾的文件
 - 3. **复制**你找到的启动文件，**重命名复制的文件**为`start-my-server.bat`（linux则重命名为`start-my-server.sh`）
 - 4. 编辑刚刚复制且重命名的文件（点击鼠标**右键**，单击编辑选项），观察文件内容中是否包含 `-gamePort`字符
   -  如果有,请将`-gamePort`后面的数字，改为第一步中的端口号
   -  如果没有，请添加` -gamePort 上面的端口 `（注意包含前后空格）
 - 5. 假设上面的端口为 `28888`，则文件内容应该为
      ```
      ...#此处是省略内容
      VRisingServer.exe ...#此处省略是内容 -gamePort 28888 
      ```
