...
!> 由于 [此原因](#pop3)，SakuraFrp 穿透的邮件服务仅可发信，不能使用 POP3 收信。
...
很显然，此地址没有指定对于邮件服务器地址 `example.com`，应使用什么端口，因为邮件服务固定使用 `25` 或 `995` 等端口。

而 SakuraFrp 并不支持将 `远程端口` 设为 `25` `995` 等低位端口，因此无法直接穿透 POP3 服务。
