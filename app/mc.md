...
1. 安装好对应版本的 Java。（一般推荐 1.13 以下用 Java 8，1.13 及以上用 Java 18）
2. 下载任意版本我的世界的服务端。（Forge 或 Fabric 等带 Mod 的服务端的初始配置过程略）
3. 编写批处理文件（Win 是 .bat，Linux 是 .sh），内容如下：
...
4. 运行一次批处理文件，此时你会发现程序立即退出，工作目录下多了一些文件，这里我们找到 `eula.txt`，在仔细阅读我的世界的 EULA 文档后，将其中的 `false` 字样改为 `true`。
5. 再次运行批处理文件，大概率会正常启动了，不过不要急，我们先暂时停止运行（指令 `stop`）。
6. 按需调整服务器配置文件，例如 `server.properties` 中的 `server-port`（默认为 25565）。
7. 创建隧道（参见 [创建隧道](/app/mc.html#%E5%88%9B%E5%BB%BA%E9%9A%A7%E9%81%93)）。
8. 愉快的开始联机吧！
