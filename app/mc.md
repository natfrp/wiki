...
- 所有人都是正版账号，不需辅助 Mod
- 所有人都登录同一个皮肤站的账号，不需辅助 Mod，具体操作参见 [此处](/offtopic/mc-java-server.md#yggdrasil-auth)
...
- 所有参与联机的玩家（包括开启局域网的主机）必须使用 **同一个** 皮肤站的账号登录游戏 (账号需自行注册)
...
::: warning 关于验证模式和存档
通过辅助 Mod 切换正版验证模式会改变玩家的 UUID 导致 **背包内物品和玩家数据丢失**，请在操作前 **备份存档** 或使用新存档游玩。  
:::
...
::: warning 关于验证模式
含有 `UUID 修复` 的选项适用于 **启用在线模式** 并且 **玩了一段时间** 后需要禁用在线模式的情况，  
若能确定有离线模式玩家，请始终保持 `在线模式` / `正版验证` 选项为 `禁用` 。请勿在世界存档使用过程中切换验证模式。
:::
...
1. 安装 Mod 并进入游戏，进入您要和朋友联机的存档
2. 按下 **ESC** 键，进入暂停菜单 (Pause Menu)，找到 `对广域网开放` 按钮并点击进入
3. 安装此 Mod 后 **广域网世界** 界面会增加下列选项，请按需配置:
  （注：您所看到的界面可能会因为版本不同而与教程有所差别，请尝试切换标签页寻找对应开关）
  ![](./_images/mc-mcwifipnp-setting.png)
4. 点击屏幕下方的 `对广域网开放`，在聊天栏中会提示 `本地游戏已在端口 xxxxx 上开启`，表示游戏内置服务端已经启动，这个五位数的端口号会固定为您在上面输入框中设置的端口号。
...
1. 安装 Mod 并进入游戏，打开您要联机的存档
2. 按下键盘上的 **ESC** 键，在出现的界面里找到 `对局域网开放` 按钮并且点击进入
...
- 隧道类型: `TCP 隧道`
- 本地 IP: `127.0.0.1`
- 本地端口: 建议设置为 `Minecraft (25565)` 开启自动探测，如有其他需求请填写游戏中显示的 `本地游戏已在端口 \[xxxxx\] 上开启` 中的端口
...
3. 检查是否为游戏网络卡顿
    ...
    您可以尝试换一个整合包或者使用原版游戏测试。也可尝试参考文档 [下一部分](#look-like-lag) 。
...
### 联机失败、成功后卡顿或延迟高 {#look-like-lag}

#### 可能原因

1. 运营商跨网 QoS 或带宽 PCDN 限速等  
    请尝试优先使用三线、多线节点。如果您和您的朋友在同一个运营商，也可以选择当前运营商的节点。

2. 服务器/主机性能不佳导致处理缓慢  
    判断方法: 让您的朋友加入游戏，在 **同一时刻** 检查 [调试屏幕](https://zh.minecraft.wiki/w/%E8%B0%83%E8%AF%95%E5%B1%8F%E5%B9%95) 页面的 `ms ticks` 项目（如安装 betterF3，则为 `内置服务器` 项目） 是否有超过 `50` ms/毫秒，如果有，即可能为主机性能不足，或模组过多等原因。  
    注: 当您不是局域网主机时可能无法看到这一项，请使用其他方法查看 tps 或 mspt 等服务器性能指标。

3. 节点超载、掉线或网络质量不佳  
    判断方法: 无准确判断方法，如果您只尝试过一个节点或者同一地区的节点，则有可能是该问题。  
    可能的解决方法: 尝试检查 [节点状态](https://www.natfrp.com/tunnel/nodes) 页面并参照其更换节点，或更换离您物理距离较近的同运营商节点。如果多个不同地区节点都卡顿基本可以排除此问题。

4. 您的网络情况不允许使用 frpc 或已经限制 sakurafrp 提供的服务  
...
5. 您的整合包对局域网联机的兼容性不佳  
    判断方法: 安装一个原版无模组客户端并尝试用其联机，或尝试在游戏设置中拉低 [渲染距离](https://zh.minecraft.wiki/w/%E9%80%89%E9%A1%B9?variant=zh-cn#video) 和 [模拟距离](https://zh.minecraft.wiki/w/%E6%A8%A1%E6%8B%9F%E8%B7%9D%E7%A6%BB) 后再试。如果能成功则有可能是此问题或第五项 (宽带上传不足) 问题。  
...
6. 宽带上传速度不足  
    判断方法: 无准确判断方法，您可以尝试在游戏设置中拉低 [渲染距离](https://zh.minecraft.wiki/w/%E9%80%89%E9%A1%B9?variant=zh-cn#video) 和 [模拟距离](https://zh.minecraft.wiki/w/%E6%A8%A1%E6%8B%9F%E8%B7%9D%E7%A6%BB) 后再进，如果可以则可能为此问题。  
