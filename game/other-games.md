...
此处收录一些我们暂时没有充足时间精力或没有必要扩展为完整教程的联机说明或 Tips，希望能给您提供一些帮助。
...
在修改配置文件时还请额外确认下面的配置以避免影响联机：

- 找到 `Public=`，确认其后面应当是 `false`，若不是请修改为 `false`
- 找到 `UPnP=`，确认其后面应当是 `false`，若不是请修改为 `false`

为避免 Steam 网络情况对联机造成干扰，我们推荐您关闭 Steam 模式（即通过 Steam 联机的模式，不影响 mod 获取）以获得稳定可用的体验：

  1. 在 Steam 游戏库中找到僵尸毁灭工程独立服务器
  1. 右键游戏图标，选择 `属性`，在 `常规` 标签页中找到 `启动选项`
  1. 在启动选项中输入 `-nosteam`，然后关闭窗口保存设置

连入时的游戏也需要关闭 Steam 模式：

  1. 在 Steam 游戏库中找到僵尸毁灭工程
  1. 右键游戏图标，选择 `属性`，在 `常规` 标签页中找到 `启动选项`
  1. 在启动选项中输入 `-nosteam`，然后关闭窗口保存设置
  1. 关闭 Steam 模式的游戏在开启时会较长时间卡在 `Initialize public servers` 界面且无法跳过，请耐心等待，这是游戏开发者的代码失误导致的。

在联机时请保证两个隧道都已开启，且游戏独立服务端已开启，游戏内服务器端口输入端口 A。
...
请在联机前正常配置并启动您的七日杀独立服务器，需进行游玩时服务器 **必须** 运行，且隧道 **必须** 开启。
...
我们注意到，某些情况下、七日杀联机中可能存在需要使用两个额外隧道的情况（共计需四个，请确认自己的隧道配额足够），如您遇到反复排查联机仍无法正常连接的情况，请参考下面的方法操作：
  ...
  1. 保证四个隧道都打开
  1. 您应当连接到主隧道（即 `_TCP` 或 `_UDP` 隧道）对应的访问地址来加入
...
灵魂面甲使用 **UDP** 协议，因此我们需要配置一个 UDP 隧道：
  ...
  1. 启动灵魂面甲独立服务器，启动隧道（在游玩过程中需保持开启）
...
### 像素工厂 (Mindustry) {#mindustry}

像素工厂在同一端口上使用 TCP/UDP [双栈端口](#tcp-udp)，映射方法为：

  1. 开始创建一个 UDP 隧道，本地端口选择 `6567`，远程端口留空以随机生成即可  
    （本地端口通常应该设置为 `6567`，如您自行设置了服务端端口，则设置为您自己的）
  1. 在点击创建按钮前，点击创建按钮左边的 `批量创建选项`，选中 `创建同端口 TCP 隧道`，然后点击创建按钮
  1. 确保创建的两个隧道都打开
  1. 在日志中找到类似 `使用 >>frp-xxx.com:11221<< 来连接到您的隧道` 的日志，点击黄色部分复制并发给客机

随后，主机打开游戏：

  1. 进入你想要联机的战役/沙盒等存档
  1. 按 `ESC` 进入暂停菜单，点击 `创建联机游戏`
  1. 设置自己在游戏内显示的游戏名称和颜色
  1. 点击 `创建` 按钮，成功后 `创建联机游戏` 按钮应当变灰

客机加入游戏：

  1. 主界面点击 `开始游戏` - `加入游戏`
  1. 在上方设置自己在游戏内显示的游戏名称和颜色
  1. 在下方点击添加服务器，地址栏输入主机发来的地址（例如 `frp-xxx.com:11221`）
  1. 点击新添加的服务器方框即可进入服务器

### 罗马拓荒录 (Romestead) {#romestead}

:::warning 游戏情况提醒和警告
此说明不构成对游戏的游戏性或安全性的任何推荐或背书，即我们不向您推荐此游戏，也不向您保证游戏文件是安全的，请自行通过游玩或其他渠道确定。

此游戏的多人联机服务器程序将消耗较多内存，请确保您的电脑有至少 6GB 的可用内存 **只** 用于运行服务端；在游玩中发现无法加入的问题，请优先检查内存以及服务器程序的运行情况。
:::

罗马拓荒录使用单一端口 `UDP/8050`，且网络库较为现代，不需要修改内外端口一致，因此配置非常简单：

  1. 创建一个 **UDP** 隧道，本地端口输入并选择 `8050`，远程端口留空即可
  1. 启动隧道，打开日志页，找到日志中的连接地址（形如`使用 >>frp-xxx.com:11221<< 来连接到您的隧道`），点击在箭头中间的部分复制连接地址，  
     其中冒号前的部分（形如 `frp-xxx.com`）为服务器 IP（称为 *域名*，此处使用时与 IP 等价，又名**地址**），冒号后的部分为**端口**。
  1. **启动联机服务器程序**，在 Steam 中搜索并运行 `Romestead Dedicated Server`，在希望其他人可以加入的时间区间中，此程序 **必须一直开启**。
  1. 在游戏中进入菜单 `多人游戏`-`加入多人游戏`-`高级/局域网` 并输入上面的信息（冒号前面是地址，冒号后面是端口）点击连接即可。

请在联机时保持 **隧道开启**，且 **服务器程序已启动**。使用独立服务器程序可以在您关闭游戏时保证其他人继续游玩，或在与游玩游戏不同的主机上运行服务器。
