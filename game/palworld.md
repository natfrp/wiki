...
简单的说就是服务器内存不足时把状态回滚了，并且 **成功地** 把这个状态 **发送到客户端**，造成本地状态被回滚
...
只要您距离服务器的赛博距离并不太远（如中国内地到其他地区，东北地区到西南地区），一个异地的服务器并不会明显影响您的游戏体验。
...
请务必确认，联机游玩时这个启动后弹出的黑框框窗口始终存在，这表示您的服务器一直在开启：

![](_images/palworld/server-cmd.png)

### 启动后的黑框框注意事项 {#console-tips}

::: tip
如果您使用的默认终端应用程序是 `Windows 终端` 而非 `Windows 控制台主机`，那么您不会出现下文提到的问题。
:::

如果您无意间在服务器运行的黑色窗口上点击了一下，此窗口将会进入选中状态。在此状态下，您的服务器将会冻结且无法游玩。此窗口的标题将会增加一个 `选择` 在最前方。

![](./_images/palworld/cmd-selecting.png)

如果您不小心选中了文本，在 **高亮此窗口的情况下** 单击鼠标右键或按下 `Esc` / `Enter` 键即可取消此状态，确认没有 `选择` 即说明您的服务端在正常运行了。

::: tip
右键同时还会将您剪切板的内容黏贴到服务器命令行中，请注意确认相关操作不会影响您服务器的安全。
:::

::: details 您可以通过以下设置防止此事件的发生（点击展开）

点击黑框框窗口左上角的图标，选择属性

![](../offtopic/_images/mc-server-tip-cmdsetting.png)

取消掉 `快速编辑模式` 的勾选

![](../offtopic/_images/mc-server-tip-cmdfastedit.png)
